 ...
 impl Bookstore {
     ...
     fn request_item(&mut self, isbn: &str) -> Option<ItemType> {
         if let Some(item) = self.items.get_mut(isbn) {
             match item {
                 ItemType::Book(b) if b.quantity > 0 => { b.quantity -= 1; Some(item.clone()) }
                 ItemType::Audiobook(a) if a.quantity > 0 => { a.quantity -= 1; Some(item.clone()) }
                 ItemType::Statue(s) if s.quantity > 0 => { s.quantity -= 1; Some(item.clone()) }
                 ItemType::Painting(p) if p.quantity > 0 => { p.quantity -= 1; Some(item.clone()) }
                 _ => None,
             }
         } else { None }
     }
     ...
     fn get_duration(&self, isbn: &str) -> Result<u32, String> {
         self.items.get(isbn).and_then(|item| {
         ...
         match input("Choose:").as_str() {
             "1" => {
                 ...
                 let item = match kind.as_str() {
                     "1" => ItemType::Book(Book { isbn, title, author, keywords, quantity }),
                     "2" => {
                         let duration = input("Duration (minutes):").parse().unwrap_or(0);
                         ItemType::Audiobook(Audiobook { isbn, title, author, keywords, quantity, duration })
                     }
                     "3" => {
                         let dims: Vec<f32> = input("Dimensions (h w d):")
                             .split_whitespace().filter_map(|v| v.parse().ok()).collect();
                         ItemType::Statue(Statue { isbn, title, author, keywords, quantity, dimensions: (dims[0], dims[1], dims[2]) })
                     }
                     "4" => {
                         let dims: Vec<f32> = input("Dimensions (h w):")
                             .split_whitespace().filter_map(|v| v.parse().ok()).collect();
                         ItemType::Painting(Painting { isbn, title, author, keywords, quantity, dimensions: (dims[0], dims[1]) })
                     }
                     _ => { println!("Invalid type."); continue; }
                 };
